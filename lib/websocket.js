...
module.exports = class Websocket extends EventEmitter {
    ...
    message(data){
        ...
        const subscriptions = this.subscriptions.filter(group => group.channel === channel);
        ...
        if(channel === 'ticker'){
            ...
            const entry = {date: new Date(), price: parseFloat(message.price)};

            for(let subscription of subscriptions.filter(group => group.id === id)){
